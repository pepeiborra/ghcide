...
main = do
    ...
    if argLSP then do
        ...
        debouncer <- newAsyncDebouncer
        sessionLoader <- loadSessionShake dir
        ide <- initialise def mainRule (pure $ IdInt 0) (showEvent lock) (logger minBound) debouncer (defaultIdeOptions sessionLoader)  vfs
...
loadSession dir = do
  ...
  case installationCheck of
    ...
    InstallationChecked compileTime ghcLibCheck -> return $ do
      ShakeExtras{logger, eventer, restartShakeSession} <- getShakeExtras
